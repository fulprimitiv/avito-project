...
export const ModerationTimeline: React.FC<ModerationTimelineProps> = ({ history }) => {
   return (
            ...
            {history.length === 0 ? (
               ...
               history.map((action) => (
                  <li key={action.id} className="moderation-timeline__item">
                     <div className="moderation-timeline__row">
                        Модератор: <b>{action.moderatorId}</b>
                     </div>
                     <div className="moderation-timeline__row">
                        Дата: <b>{formatDate(action.timestamp)}</b>
                     </div>
                     <div className="moderation-timeline__row">
                        Решение: <b>{action.reason}</b>
                     </div>
                     {action.comment && (
                        <div className="moderation-timeline__row">
                           Комментарий: <b>{action.comment}</b>
                        </div>
                     )}
                  </li>
               ))
